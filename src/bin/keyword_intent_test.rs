...
/// Check if a string is purely Thai characters
fn is_pure_thai(s: &str) -> bool {
    !s.is_empty() && s.chars().all(|c| ('\u{0E00}'..='\u{0E7F}').contains(&c))
}

/// Check if a string is purely ASCII (English/numbers/symbols)
fn is_pure_ascii(s: &str) -> bool {
    !s.is_empty() && s.chars().all(|c| c.is_ascii())
}

/// Extract the leading English/alphanumeric prefix from a mixed string
/// e.g., "888NEO" -> "888", "GOALสล็อต" -> "GOAL"
...
/// Extract the leading Thai prefix from a mixed string
...
/// Strategy:
/// 1. Get the first word (split by space)
/// 2. If 100% Thai → use as-is
/// 3. If 100% English/ASCII → use as-is
/// 4. If mixed Thai/English → extract the dominant prefix
fn extract_prefix_intent_keyword(text: &str) -> Option<String> {
    ...
    if has_thai && has_ascii {
        // Mixed: extract the leading portion
        ...
        if first_char_is_thai {
            ...
            if thai_prefix.len() >= 2 {
                ...
                // Thai part is too short, use the ASCII part
            ...
            // Starts with ASCII - extract the leading alphanumeric portion
...
/// Lazy-initialized BPE tokenizer (cached to avoid re-loading)
...
fn subtokenize_keyword(keyword: &str) -> Vec<String> {
    ...
    // If tiktoken didn't split anything, return original
...
// ── RAKE Keyword Extraction (intent dictionary scanning) ──────────

/// Extract keywords from body text by scanning intent dictionary
/// Instead of relying on tiktoken (which doesn't segment Thai well),
/// we scan the text for known intent keywords from the dictionary.
pub fn rake_keywords(text: &str, top_n: usize) -> Vec<(String, f64)> {
    ...
    for word in lower.split_whitespace() {
        ...
        if cleaned.len() >= 2 && !THAI_STOP_WORDS.contains(&cleaned) {
            // Check if this word contains any intent dictionary substring
            for (_intent, dict_words) in INTENT_DICT.iter() {
                for &dict_word in dict_words {
...
pub fn extract_keyword_intents(text: &str) -> IntentResponse {
    ...
    // Technique 2: RAKE for the rest of the text
...
fn main() {
    println!("=== Keyword Intent Extraction - Testing against data/spam.txt ===");
    println!("Technique 1: Prefix keyword extraction (first word before space)\n");
    ...
    println!("Processing {} spam samples...\n", lines.len());
    ...
    for (idx, line) in lines.iter().enumerate() {
        ...
        *intent_counts.entry(result.primary_intent.clone()).or_insert(0) += 1;
        ...
        if idx < 25 {
            // Extract and show the prefix keyword
            let prefix_kw = extract_prefix_intent_keyword(line);
            let sub_tokens = prefix_kw.as_ref()
                .map(|kw| subtokenize_keyword(kw))
                .unwrap_or_default();
            
            println!("[Sample {}]", idx + 1);
            println!("  Input: \"{}\"", truncate(line, 80));
            println!("  Prefix keyword: {:?} → sub-tokens: {:?}", prefix_kw, sub_tokens);
            println!("  Primary Intent: {} (confidence: {:.0}%)", 
                results[idx].primary_intent, 
                results[idx].confidence_score * 100.0);
            println!("  Keywords:");
            let kw_count = results[idx].extracted_keywords.len().min(6);
            for kw in &results[idx].extracted_keywords[..kw_count] {
                println!("    - {:<20} score: {:.2}", kw.word, kw.score);
            }
            println!();
        }
    }
    ...
    println!("Total samples analyzed: {}", lines.len());
    println!("Total keywords extracted: {}", total_keywords);
    println!("\nIntent Breakdown:");
    ...
    for (intent, count) in &intents {
        ...
        println!("  {:<20} {:>4} samples ({:.1}%)", intent, count, percentage);
    }

    println!("\n\n=== Sample JSON Output (first result) ===");
    if let Some(first_result) = results.first() {
        let json = serde_json::to_string_pretty(first_result).unwrap();
        println!("{}", json);
    }
    ...
    println!("Processed 100 iterations in {:?}", duration);
    println!("Average: {:.2}ms per iteration (target: <50ms)", avg_ms);
}
